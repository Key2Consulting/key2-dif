...
-- Description:	Used to fetch Excel file to parquet task data. 
...
SELECT * FROM [DIF].[udf_GetDITaskExcelFileSheetList]('Excel File')

*/


-- =============================================
CREATE FUNCTION [DIF].[udf_GetDITaskExcelFileSheetList] 
(	
...
(
	...
	SELECT
		...
		SourceExcelFileKey = EF.ExcelFileKey,
		SourceExcelFileSheetName = EF.ExcelFileSheetName,
		SourceExcelFileSheetFirstRowAsHeader = EF.ExcelFileSheetFirstRowAsHeader,
		SourceExcelFileSheetIsHeaderRowOverride = ISNULL(EF.ExcelFileSheetIsHeaderRowOverride, 0),
		SourceExcelFileSheetRange = EF.ExcelFileSheetRange,
		SourceExcelFileNamePattern = EF.ExcelFileNamePattern,		
		SourceExcelFileCompressionType = EF.ExcelFileCompressionType,		
		SourceExcelFileNULLValue = EF.ExcelFileNULLValue,
	...
	FROM
		...
		LEFT JOIN [metadata].[ExcelFile] EF
			ON EF.DatasetKey = TD.SourceDatasetKey
