...
-- Description:	Used in "ExcelFile_RawToRefined" pipeline to fetch details for raw zone files ultimately sourced from delimited files.  

/*


SELECT * FROM [DIF].[udf_GetRawExcelFileDetails](131)

*/


-- =============================================
CREATE FUNCTION [DIF].[udf_GetRawExcelFileDetails] 
(	
	@DITaskKey SMALLINT
...
(
	...
	(
		SELECT
			...
			SourceDatabaseName = REPLACE(td.DestinationRepositoryName, ' ', '_'),  
			tableSchema = REPLACE(td.DestinationDatasetNameSpace, ' ', '_'), 
			tableName = REPLACE(td.DestinationDatasetName, ' ', '_'),  
	...
	(		
		SELECT
			...
			tableSchema = REPLACE(DIT2.DestinationDatasetNameSpace, ' ', '_'), 
			tableName = REPLACE(DIT2.DestinationDatasetName, ' ', '_')
		FROM
		(
			SELECT
				L.DITaskKey,
		...
		LEFT JOIN  [DIF].[DITaskDetail] DIT2 
			ON DIT2.DITaskKey = L2.DITaskKey
		WHERE
			L2.PartionKeySeq = 1		
	)
	...
	SELECT 
		...
		UniqueIDColumnList = ISNULL(rdit.UniqueIDColumnList, 'NULL'),  -------for ADF debug mode!!
